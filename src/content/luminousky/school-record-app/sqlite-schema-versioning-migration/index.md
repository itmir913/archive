...
이 글은 스키마 버전이 1이던 시점의 기록이다. 그 뒤로 두 가지가 변경되었다.

**백업과 마이그레이션의 순서가 뒤집혔다.** 위의 "open과 migrate의 분리" 절은 백업을 먼저 생성하고 마이그레이션을 실행하는 흐름을 전제했다. 메모를 암호화하는 마이그레이션이 도입되면서 그 순서는 유지할 수 없게 되었다. 변환 전 사본에는 평문 메모가 포함되므로, 본 DB만 암호화하고 비밀번호 없이 읽히는 사본을 옆에 영구히 남기는 결과가 발생한다. 현재는 마이그레이션을 먼저 실행하고 그 뒤에 백업을 생성하며, 백업 커맨드가 마이그레이션 완료를 직접 요구한다. 경위는 [마이그레이션 이후의 실패는 열기를 차단하지 않는다](../failures-that-must-not-block-open/)에 기록했다.

마이그레이션 실행 시점도 사용자 동의 다이얼로그가 아니라 파일을 여는 흐름의 한 단계다.

**`foreign_key_check`의 범위를 축소했다.** 위 코드는 모든 단계에서 커밋 전에 검사를 실행한다. v1에서 v2로 가는 마이그레이션에는 DDL이 없는데, `SCHEMA_VERSION`이 계속 1이었던 탓에 배포된 파일에서는 이 검사가 한 번도 실행된 적이 없었다. 그 상태로 전면 검사를 도입하면, 원래부터 고아 행이 있던 파일은 열 때마다 같은 지점에서 정지한다. 현재는 DDL을 실제로 실행한 단계로 한정한다.

`SCHEMA_VERSION`은 2이고, 변경 절차 자체는 [스키마 지문으로 버전 누락을 차단하다](../schema-fingerprint-lock/)와 [DDL이 동일한데 버전을 올린 마이그레이션](../memo-encryption-schema-v2/)에 이어진다.
